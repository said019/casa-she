...
const CATALOG: OnboardingCatalog = {
  disciplines: {
    'Pilates Mat': { id: 'ct-pm' }, Barre: { id: 'ct-barre' }, Sculpt: { id: 'ct-sculpt' },
    Yoga: { id: 'ct-yoga' }, Flex: { id: 'ct-flex' }, Salsa: { id: 'ct-salsa' },
  },
  plans: {
...
// (1) Principiante + tonificar + core: Pilates Mat domina; Sculpt se excluye sin reto.
{
  ...
  assert.ok(!names(scored).includes('Sculpt'), 'Sculpt se excluye para principiante sin intensidad retador');
}

// (2) Embarazo: excluye Sculpt y prioriza prácticas de control/movilidad.
{
  ...
  const scored = scoreDisciplines(a, DEFAULT_RULES);
  assert.ok(!names(scored).includes('Sculpt'), 'embarazo excluye Sculpt');
  assert.ok(names(scored).slice(0, 3).includes('Pilates Mat'), 'embarazo prioriza Pilates Mat');
  assert.ok(names(scored).slice(0, 3).includes('Yoga'), 'embarazo prioriza Yoga');
}

// (3) Lesión: baja Sculpt y sube Pilates Mat/Flex.
{
  const a: OnboardingAnswers = { goal: 'probar', level: 'avanzada', body_focus: ['brazos'], intensity: 'retador', frequency: '3x', health: ['lesion'] };
  const scored = scoreDisciplines(a, DEFAULT_RULES);
  const sculpt = scored.find((s) => s.name === 'Sculpt');
  const flex = scored.find((s) => s.name === 'Flex');
  assert.ok(sculpt, 'Sculpt sigue disponible para avanzada+retador, con menor puntaje');
  assert.ok(names(scored).slice(0, 3).includes('Pilates Mat'), 'lesión sube Pilates Mat al top 3');
  assert.ok(flex && flex.score >= sculpt.score, 'lesión prioriza Flex al menos al nivel de Sculpt');
}
...
// (6) Devuelve mínimo 2 disciplinas, Salsa como experiencia, plan por frecuencia, ids del catálogo.
{
  const a: OnboardingAnswers = { goal: 'tonificar', level: 'principiante', body_focus: ['core'], intensity: 'equilibrado', frequency: '1x', health: ['ninguna'] };
  ...
  assert.equal(rec.experience?.name, 'Salsa', 'experiencia = Salsa');
...
// (8) Embarazo → requires_clearance y no recomienda Sculpt.
{
  ...
  assert.ok(!rec.disciplines.some((d) => d.name === 'Sculpt'));
