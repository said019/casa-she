...
const normalizeSw = text => text.replace(/const VERSION = '[^']*';/, "const VERSION = 'normalized';");
assert.equal(normalizeSw(await readFile('public/sw.js', 'utf8')), normalizeSw(await readFile('dist/sw.js', 'utf8')));
assert.match(await readFile('dist/sw.js', 'utf8'), /const VERSION = 'bmb-[^']+';/);
...
async function start(binary, args) {
    ...
    const child = spawn(binary, args(port), { env: { ...process.env, PORT: String(port), NO_UPDATE_CHECK: '1' }, stdio: 'ignore' });
    children.push(child);
...
try {
    ...
    const caddy = await start(process.env.CADDY_BIN || './.runtime/caddy', () => ['run', '--config', 'Caddyfile', '--adapter', 'caddyfile']);
    ...
    const js = files.find(p => p.endsWith('.js') && p.startsWith('assets/index-'));
    ...
    for (const path of ['/sw.js', '/icon-192.png', '/casashe/logo-wordmark.png', '/' + js]) {
    ...
    console.log(JSON.stringify({ result: 'PASS', routes: routes.length, files: files.length,
        equivalentGetPaths: paths.length - 2, originalFilesUnchanged: true, workerLogicUnchangedWithBuildStamp: true,
