...
└── <Schedule />                    src/components/Schedule.tsx
    ...
    │
    └── CTA "¿Primera vez en Balance Room?"
```
...
// Si el nombre no coincide → '#A48550' (dorado Balance Room)
```
...
|-----------|--------|------------|
| Día seleccionado | Fondo `balance-room-gold`, texto blanco, escala 1.03 | Sí |
| Hoy (no seleccionado) | Borde `balance-room-gold/30`, número dorado | Sí |
...
```
┌────────────────────────────────────────────────────────────┐
│  bg-gradient-to-br from-balance-room-dark via-[#3D3229]        │
│                                                            │
│  Decoradores: 3 blobs de luz difusa (absolute)             │
│                                                            │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  [label pequeño] BALANCE ROOM STUDIO                     │  │
...
| Hover | `shadow-md`, `-translate-y-0.5`, borde `balance-room-gold/30` | — |
| En curso (hoy) | `ring-2 ring-balance-room-gold/40` | `[Reservar]` |
...
```
bg-gradient-to-br from-balance-room-cream to-balance-room-sand/20
border border-balance-room-sand/30
...
|-------|-----------------|
| `balance-room-dark` | Oscuro café-negro |
| `balance-room-gold` | Dorado (#A48550 aprox.) |
| `balance-room-sand` | Arena cálida |
| `balance-room-olive` | Oliva suave |
| `balance-room-cream` | Crema claro |
