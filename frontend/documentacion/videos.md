...
4. [Almacenamiento — Google Drive y Cloudinary](#4-almacenamiento--google-drive-y-cloudinary)
...
┌─────────────────────────────────────────────────────────────────────┐
│                       ALMACENAMIENTO                                │
│  Google Drive (principal): OAuth2 refresh, permisos reader public   │
│  Cloudinary (opcional): authenticated type, signed URLs (2h TTL)   │
...
CREATE TABLE videos (
    ...
    description         TEXT,
    cloudinary_id       VARCHAR(500),        -- ID en Cloudinary (si se usa Cloudinary)
    drive_file_id       VARCHAR(500),        -- ID del archivo en Google Drive (almacenamiento principal)
...
### Google Drive (almacenamiento principal)
```env
...
> ⚠️ **Nota importante:** El backend detecta automáticamente qué proveedor usar. Si `CLOUDINARY_CLOUD_NAME` + `CLOUDINARY_API_KEY` + `CLOUDINARY_API_SECRET` están definidas, usa Cloudinary. **De lo contrario usa Google Drive** (que es el almacenamiento activo en producción). Para forzar Drive basta con NO definir las variables de Cloudinary.

### Cloudinary (opcional, si se configura tiene prioridad)
```env
...
## 4. Almacenamiento — Google Drive y Cloudinary

El sistema soporta dos proveedores de almacenamiento. **Google Drive es el activo en producción.** Cloudinary se usa si sus variables de entorno están presentes (tiene prioridad cuando está configurado).

### 4.1 Google Drive (almacenamiento activo en producción)

**Variables requeridas:** `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REFRESH_TOKEN`, `GOOGLE_DRIVE_FOLDER_ID`

**Flujo interno (`uploadBufferToGoogleDrive`):**
```typescript
// 1. Obtener access token por OAuth2 refresh
POST https://oauth2.googleapis.com/token
{ client_id, client_secret, refresh_token, grant_type: 'refresh_token' }
// → devuelve access_token

// 2. Subir el archivo (multipart a Drive API)
POST https://www.googleapis.com/upload/drive/v3/files
    ?uploadType=multipart
    &fields=id,webViewLink,thumbnailLink,videoMediaMetadata
Authorization: Bearer {access_token}
// Cuerpo: multipart/related con metadata JSON + buffer del archivo

// 3. Hacer el archivo público (cualquier persona puede ver)
...
{
    fileId: string,
    webViewLink: string,   // https://drive.google.com/file/d/{id}/view
    thumbnailUrl: string,  // https://drive.google.com/thumbnail?id={id}&sz=w640
    durationSeconds: number
}
```

**URLs de reproducción en Drive:**
- **Embed:** `https://drive.google.com/file/d/{fileId}/preview` — se usa en iframe para reproducción
- **Descarga directa:** `https://drive.google.com/uc?export=download&id={fileId}`
- **Thumbnail:** `https://drive.google.com/thumbnail?id={fileId}&sz=w640`
- **Ver en Drive:** `https://drive.google.com/file/d/{fileId}/view`

**Columnas en BD cuando se usa Drive:**
- `drive_file_id` = `fileId` de Google Drive
- `cloudinary_id` = también se guarda el `fileId` (mismo valor) para compatibilidad
- `thumbnail_url` = URL de thumbnail de Drive o la generada por Drive
- `thumbnail_drive_id` = `fileId` del thumbnail si se subió uno separado

### 4.2 Cloudinary (opcional, tiene prioridad si está configurado)
...
**Cuando se usa Cloudinary:** Los videos se suben con `type: 'authenticated'` — sin URL pública. Solo accesibles con URLs firmadas que expiran cada 2 horas.

### 4.3 Detección automática en el backend
...
if (isCloudinaryConfigured) {
    // → Cloudinary tiene prioridad
} else {
    // → Google Drive (activo en producción)
}
```
...
**Lógica de upload:**
1. Si las 3 variables de Cloudinary están definidas → sube a Cloudinary (`resource_type: 'video', type: 'authenticated'`)
2. **Si Cloudinary NO está configurado → sube a Google Drive** (flujo activo en producción):
   - Obtiene token OAuth2 haciendo `POST https://oauth2.googleapis.com/token` con el `GOOGLE_REFRESH_TOKEN`
   - Sube el archivo con `POST https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart`
   - Si `GOOGLE_DRIVE_FOLDER_ID` está definido, el archivo se sube dentro de esa carpeta
   - Llama a `makeGoogleDriveFilePublic(fileId)` → agrega permiso `{ role: 'reader', type: 'anyone' }`
   - El thumbnail sigue el mismo camino: se sube a Drive y se genera URL tipo `https://drive.google.com/thumbnail?id={id}&sz=w640`
3. Si ninguno está configurado → responde `503 Carga de videos no configurada`

**Response (Google Drive):**
```json
{
    "cloudinary_id": "1AbCdEfGhIjKlMnOpQ",
    "drive_file_id": "1AbCdEfGhIjKlMnOpQ",
    "thumbnail_url": "https://drive.google.com/thumbnail?id=1AbCdEfGhIjKlMnOpQ&sz=w640",
    "thumbnail_drive_id": "",
    "duration_seconds": 1847,
    "secure_url": "https://drive.google.com/file/d/1AbCdEfGhIjKlMnOpQ/view"
}
```

> **Nota:** Cuando se usa Drive, `cloudinary_id` y `drive_file_id` contienen el **mismo valor** (el `fileId` de Drive). La columna `cloudinary_id` en la BD queda con el ID de Drive como string.
